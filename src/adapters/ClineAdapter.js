...
export class ClineAdapter {
  ...
  static async scan() {
    ...
    const possiblePaths = [
      ...
      path.join(appdata, 'Code - Insiders', 'User', 'globalStorage', 'saoudrizwan.claude-dev', 'tasks')
    ];
    ...
    for (const tasksDir of possiblePaths) {
      ...
      try {
        ...
        for (const tid of taskDirs) {
          ...
          // Read ui_messages.json or api_conversation_history.json
          ...
          try {
            ...
            if (totalTokens > 0 || firstPrompt) {
              records.push({
                id: `cline_${tid.slice(0, 12)}`,
                tool: 'Cline',
                sessionName: `Cline: ${firstPrompt || `任务 #${tid.slice(0, 8)}`}`,
                timestamp,
